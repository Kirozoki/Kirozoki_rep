...
vector<Document> FindTopDocuments (const string& raw_query, const DocumentStatus& sort_status=DocumentStatus::ACTUAL) const {
        return FindTopDocuments(raw_query, [sort_status](int document_id, DocumentStatus status, int rating){return status== sort_status;});
        }
...
private:
    ...
    map<int, DocumentData> documents_; // documents_.at(document_id)
